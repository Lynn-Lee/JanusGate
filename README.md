...
Phase 4 #t46 已启动会话录制与命令检索后端基础：新增租户隔离的 `SessionRecording` 与 `SessionCommandEvent` 持久化模型，并提供 `POST /api/v1/sessions/{session_id}/recordings`、`POST /api/v1/session-recordings/{recording_id}/commands`、`POST /api/v1/session-recordings/{recording_id}/close` 与 `GET /api/v1/session-recordings/commands?query=...`。命令事件和录制关闭按当前租户收敛，跨租户或已关闭录制返回 `SESSION_RECORDING_NOT_FOUND`，命令输出摘要会脱敏 token/password/secret/credential 赋值文本；实时上报通道、回放 UI 与全文索引优化留后续 #t46 切片。
