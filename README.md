...
Phase 6 #t75 在 #t47 WebHook 投递队列上扩展渠道类型：钉钉 / 飞书 / Lark / 企业微信 / Slack 只允许官方 host，机器人 token 从 URL 剥离后加密落库；邮件与短信走 HTTPS 网关 Bearer，不直连 SMTP；站内信只对当前用户可见。系统消息订阅与 `POST /api/v1/notification-events/` 按规则和订阅扇出，失败仍走重试与死信。设置页提供「通知渠道 / 系统消息订阅 / 站内信」。详见 [`docs/site/notification-channels.md`](docs/site/notification-channels.md)。
