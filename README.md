...
Phase 6 #t79 已落地租户资源标签（资产绑定）、白名单动态系统配置与变更修订、用户偏好、SHA-256 泄露密码库（接入创建/改密），以及包装现有审计汇总 / SOC2 导出的报表目录。详见 [`docs/site/platform-governance.md`](docs/site/platform-governance.md)。
