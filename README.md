...
### M6 审计类型（#t78 分类日志 QA SHIP）

Phase 6 #t78 把 JumpServer 的 OperateLog / ActivityLog / FTPLog / PasswordChangeLog / JobLog / UserSession 接到已有的 #t61 append-only hash chain：分类只体现在 `audit_events.category`，不另建可绕过账本的表。连接器 `POST /api/v1/connectors/{id}/file-transfers` 入库 SFTP 传输（失败也落账）；登录改密与账号凭据轮换写改密日志（无明文 / secret_id）；Ansible 终态写作业日志（丢弃 stdout/stderr/inventory）。控制台 `/audits` 增加分类标签页与在线会话（不返回 connection_url / token）。会话共享、端点路由、命令/录像多存储后端（S3/OSS/ES）明确延后。详见 [`docs/site/audit-types.md`](docs/site/audit-types.md)。
