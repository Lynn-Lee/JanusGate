...
Phase 4 #t44 已启动 SSH CA / 临时证书后端基础：新增 `SshCertificateAuthority` 与 `SshCertificate` 持久化模型，`Asset` 可绑定信任的 SSH CA，`SshCertificateService` 负责按租户、资产信任配置与 SSH account 边界签发短期证书，并支持同租户 issued certificate 撤销。当前已暴露 `GET/POST /api/v1/ssh-certificate-authorities/`、`POST /api/v1/ssh-certificate-authorities/{authority_id}/disable`、`GET/POST /api/v1/ssh-certificates/` 与 `POST /api/v1/ssh-certificates/{certificate_id}/revoke`，按当前用户租户和账号可见范围收敛，响应不返回 CA 私钥材料或 CA 私钥 secret 引用。后端已新增 Vault-backed `VaultOpenSshCertificateSigner` service primitive，可用 CA 私钥 secret 引用生成可解析的 OpenSSH user certificate；API 路由切换真实 Vault provider、连接器信任分发和前端入口仍在后续 #t44 切片中完成。
