...
Phase 6 #t77 已落地作业中心：`OpsPlaybook` / `OpsJob` / `OpsJobVariable` / `OpsJobExecution`，API 前缀 `/api/v1/ops/`。队列任务类型 `ops.job` 的 payload 只含 `execution_id` 与 `check_mode`，命令、变量和 runas 从执行记录加载，不使用 pickle。临时命令走内置 `deploy/ansible/playbooks/janusgate-adhoc.yml`，入队前执行命令过滤 ACL，DENY / REVIEW 均拒绝。控制台入口为「作业中心」。详见 [`docs/site/job-center.md`](docs/site/job-center.md)。
