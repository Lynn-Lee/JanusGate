...
### M3 真实连接通道（SSH / K8s 已走通；生产 resolver 已 SHIP）
...
### M5 工单、通知、认证源（#t74 / #t75 / #t76 已完成）

Phase 6 #t75 已在 #t47 WebHook / 通知规则 / 投递队列上扩展 `ChannelNotificationSender`：钉钉 / 飞书 / Lark / 企微 / Slack / 短信 HTTPS 网关 / 邮件 HTTPS 网关 / 站内信。IM 只接受官方 host，机器人 token 从 query/路径剥离后 AES-256-GCM 落库，列表 URL 不含凭据。系统消息订阅把事件类型扇出到当前租户渠道；`GET /api/v1/in-app-messages/` 只返回当前用户站内信。失败路径继续进 dead-letter，错误信息不含 payload、凭据或下游响应体。邮件 SMTP 直连不在本切片。详见 [`docs/site/notification-channels.md`](docs/site/notification-channels.md)。
