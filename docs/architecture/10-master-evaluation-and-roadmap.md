...
2. **策略持久化**：Phase 4 #t48 已启动 `ApprovalPolicyModel` 的租户隔离管理 API，`GET/POST /api/v1/workflows/approval-policies` 可创建和列出当前租户的 approval policy template，并要求 `workflow:admin` 或 `admin` 权限；`PolicyDecisionService` 已可接收 approval policy template，并把匹配 selector 的请求 fail-closed 转为 `APPROVAL_REQUIRED` obligations；`POST /api/v1/workflows/approval-policies/simulate` 已可在当前租户内复用同一决策引擎做策略模拟；approval policy family/version 基础已完成，`POST /api/v1/workflows/approval-policies/{policy_id}/versions` 可在当前租户内创建递增版本并停用旧 active 版本，列表与模拟默认只读取 active/latest 版本；灰度/回滚和 DSL 执行仍待后续切片
...
**Phase 4 增强**：
- SecretProvider 接口抽象（DBEncryptedProvider / KMSProvider / VaultProvider）
- envelope encryption（KEK + DEK per record）
- 审批后 unwrap
- break-glass 控制
- 凭据轮换调度 + 双写迁移 + 回滚
...
| **#t48** | JIT 策略模板、审批策略 DSL | architect + backend | `ApprovalPolicyModel` 已通过租户隔离的 `GET/POST /api/v1/workflows/approval-policies` 提供策略模板创建和列表 API，要求 `workflow:admin` 或 `admin` 权限；`PolicyDecisionService` 已可接收 approval policy template 并对匹配 selector 的请求返回 `APPROVAL_REQUIRED` obligations；`POST /api/v1/workflows/approval-policies/simulate` 已可在当前租户内复用同一决策引擎做策略模拟；approval policy family/version 基础已完成，`POST /api/v1/workflows/approval-policies/{policy_id}/versions` 可在当前租户内创建递增版本并停用旧 active 版本，列表与模拟默认只读取 active/latest 版本；后续补灰度/回滚和 DSL 执行 | 中 |
