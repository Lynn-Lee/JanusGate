...
6. **通知/WebHook 集成**：Phase 4 #t47 已启动租户隔离的 `WebhookEndpoint` 持久化模型与 `GET/POST /api/v1/webhook-endpoints/` 管理 API；响应不返回 signing secret 明文或摘要。当前已补 `NotificationRule` 持久化模型与 `GET/POST /api/v1/notification-rules/` 管理 API，规则必须引用当前租户 active WebHook endpoint；并补 `NotificationDelivery` 队列记录与租户隔离的入队/列表 API。`NotificationDeliveryWorker` 已提供到期投递、失败重试与最大次数后 dead-letter 状态机，sender 只接收已脱敏 payload。Phase 6 #t75 已落地 `ChannelNotificationSender`（钉钉 / 飞书 / Lark / 企微 / Slack / SMS / 邮件 HTTPS 网关 / 站内信）、系统消息订阅与当前用户站内信；IM 仅官方 host，凭据加密落库不回显。多级审批已由 #t74 完成。SMTP 直连邮件可后续切片。
...
| **#t75** | 通知渠道扩展 | backend | **✅ 已完成**：`WebhookEndpoint.channel_type` / `credential_encrypted` / `recipient`；`SystemMessageSubscription`；`InAppMessage`；迁移 `f6a7b8c9d0e1`。`ChannelNotificationSender` 按渠道分发钉钉 / 飞书 / Lark / 企微 / Slack / SMS / 邮件 HTTPS 网关 / 站内信。IM 仅官方 host，机器人 token 从 URL 剥离后 AES-256-GCM 落库，列表 URL 不含凭据。邮件本切片走 HTTPS 网关（SMTP 直连可后续）。`POST /api/v1/notification-subscriptions/`、`POST /api/v1/notification-events/`、`GET /api/v1/in-app-messages/`。设置页 `NotificationChannelPanels`。测试 `test_notification_channels_t75.py`。**通知渠道扩展已 QA SHIP**（沿用 #t47 脱敏 payload 与 dead-letter；失败信息不含 payload/凭据/下游响应体） | 中 |
...
> 因此后续排序建议为：**#t77 作业中心**（#t66/#t67/#t68、#t69/#t72、**#t73 AccountTemplate / account.verify**、**#t74 asset_grant / command_review TicketFlow**、**#t75 通知渠道扩展** 与 **#t76 OIDC 登录** 已 QA SHIP；#t70 建议独立立项），而非按里程碑编号顺序推进。
...
| 14 | 通知 | 🟡 WebHook endpoint + 通知规则 + 投递队列 + 重试/死信 + HTTPS/IM/SMS/邮件网关/站内信 sender + 系统消息订阅（#t75 QA SHIP） | SMTP 直连邮件、更广订阅管理 UI | 是 | P4 #t47 → **P6 #t75 已完成** |
