...
| 任务 ID | 任务 | Owner | 范围 | 优先级 |
|---------|------|-------|------|--------|
| **#t77** | 作业中心 | backend + frontend | **✅ 已完成**：`OpsPlaybook` / `OpsJob` / `OpsJobVariable` + `AutomationJobRun` 扩展；`job.playbook` / `job.adhoc` 入 `ALLOWED_JOB_TYPES`；队列 payload 仅 `job_id`（JSON-only，禁止 pickle）；runas 仅 SSH 账号用户名进 inventory；参数化拒绝敏感键与 Jinja；5 段 cron + `POST /api/v1/job-center/scheduler/tick`；复用 #t52 Ansible runner（临时命令走 runtime 内联 playbook + extra-vars 文件）。控制台 `/jobs`。文档 `docs/site/job-center.md`。**作业中心已 QA SHIP**（P0#10 不得回退）。真实云上大规模并发调度与更广 runas 策略可后续切片 | 中 |
...
> 因此后续排序建议为：**#t78 审计类型补全**（#t66/#t67/#t68、#t69/#t72、**#t73 AccountTemplate / account.verify**、**#t74 asset_grant / command_review TicketFlow**、**#t76 OIDC 登录** 与 **#t77 作业中心** 已 QA SHIP；#t75 通知渠道可与本切片并行合入；#t70 建议独立立项），而非按里程碑编号顺序推进。
...
| 15 | 作业中心 | 🟡 JSON-only 队列 + worker + Ansible runner + 执行记录 + **#t77 Job/Playbook/AdHoc/变量/cron/runas QA SHIP** | 更广 runas 策略、大规模调度器与 Playbook 内容治理 | 是 | P4 #t52 → **P6 #t77** |
