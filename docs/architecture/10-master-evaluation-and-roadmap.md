...
6. **通知/WebHook 集成**：Phase 4 #t47 已启动租户隔离的 `WebhookEndpoint` 持久化模型与 `GET/POST /api/v1/webhook-endpoints/` 管理 API；响应不返回 signing secret 明文或摘要。当前已补 `NotificationRule` 持久化模型与 `GET/POST /api/v1/notification-rules/` 管理 API，规则必须引用当前租户 active WebHook endpoint；并补 `NotificationDelivery` 队列记录与租户隔离的入队/列表 API。`NotificationDeliveryWorker` 已提供到期投递、失败重试与最大次数后 dead-letter 状态机，sender 只接收已脱敏 payload。**#t75 已扩展**钉钉 / 飞书 / Lark / 企微 / Slack / SMS / 邮件 / 站内信与系统消息订阅。
...
| **#t75** | 通知渠道扩展 | backend | **✅ 已完成**：在 #t47 WebHook / 通知规则 / 投递队列上扩展 `channel_type`（webhook / dingtalk / feishu / lark / wecom / slack / sms / email / inbox）；IM 仅官方 host、URL 禁止持久化 query；渠道凭据 AES-256-GCM；`SystemMessageSubscription` + `POST /api/v1/notification-events/` 扇出；站内信 `InAppMessage`；邮件 SMTPS/STARTTLS；SMS 为 HTTPS 网关 adapter。设置页「通知渠道 / 系统消息订阅 / 站内信」。文档 `docs/site/notification-channels.md`。**通知渠道已 QA SHIP**（沿用脱敏 payload 与 dead-letter）。真实运营商 SMS 专有协议与 SMTP 账号池可后续切片 | 中 |
...
| 14 | 通知 | 🟡 WebHook + 通知规则 + 投递队列 + 重试/死信 + **#t75 IM / SMS / 邮件 / 站内信 / 系统消息订阅 QA SHIP** | 真实运营商 SMS 专有协议、SMTP 账号池 | 是 | P4 #t47 → **P6 #t75** |
