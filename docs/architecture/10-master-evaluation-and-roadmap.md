...
| **#t78** | 审计类型补全与会话高级能力 | backend + frontend | **✅ 分类日志 QA SHIP**：JumpServer OperateLog / ActivityLog / FTPLog / PasswordChangeLog / JobLog / UserSession 全部并入 #t61 hash chain + WORM，**不另建可绕过账本的表**。`AuditCategory` 增 `operate` / `activity` / `file_transfer` / `password_change` / `job`；迁移 `f7a8b9c0d1e2`（`ix_audit_events_tenant_category`）。`POST/GET /api/v1/audits/operate-logs`、`activity-logs`（列表含 `activity`+`auth`+`session`）、`GET file-transfers` / `password-changes` / `job-logs`；`GET /events?category=`；`GET /online-sessions`（`get_read_db`，不返回 `connection_url` / token）。连接器 `POST /api/v1/connectors/{id}/file-transfers`（跨租户/非 active → 404 `CONNECTOR_NOT_FOUND`）；`HttpFileTransferEventSink` + 生产调度器 `AuditServiceFileTransferSink`。登录改密、凭据轮换终态写改密日志（无明文 / `secret_id`）；Ansible 终态写作业日志（丢弃 stdout/stderr/inventory）。控制台 `/audits` 分类标签页。测试 `test_audit_types_t78.py` + `test_file_transfer_sink.py`。**明确延后**：会话共享与监控联机、连接端点路由、命令/录像多存储后端（S3/OSS/ES） | 中 |
...
| M6：运维与平台治理 | 3-4 周 | #t77 + #t78 + #t79 | 🟡 #t78 分类日志 QA SHIP；#t77/#t79 未开始 | 收口阶段；会话共享/端点/ES 延后 |
...
> 因此后续排序建议为：**#t79 平台治理**（#t66/#t67/#t68、#t69/#t72、**#t73 AccountTemplate / account.verify**、**#t74 asset_grant / command_review TicketFlow**、**#t76 OIDC 登录** 与 **#t78 分类审计** 已 QA SHIP；#t75 通知渠道与 #t77 作业中心若尚未合入可并行；#t70 建议独立立项；#t71 DB 代理可并行），而非按里程碑编号顺序推进。
...
| 13 | 审计 | 🟡 统一审计事件 + **已持久化 append-only** + 库层强制有序 hash chain + SIEM + 合规报表 + WORM 归档 + **分类日志（#t78 QA SHIP：操作/活动/文件传输/改密/作业）** + 在线 PAM 会话 | 会话共享与监控联机、端点路由、命令/录像多存储后端（S3/OSS/ES） | 是 | P1 已有 → **P6 #t61、#t78** |
