...
| 任务 ID | 任务 | Owner | 范围 | 优先级 |
|---------|------|-------|------|--------|
| **#t77** | 作业中心 | backend + frontend | **✅ 已完成（首切片）**：`JobPlaybook` 目录（相对路径 jail + 存在性校验）/ `Job` 定义（playbook-only）/ `JobExecution`（经 `message_id` 关联 `AutomationJobRun`）；`POST /api/v1/job-center/jobs/{id}/run` 复用 #t52 `ansible.playbook` JSON-only 队列（载荷可含 `job_execution_id`，无凭据）；轻量运行时覆盖 `target_asset_ids` / `check_mode`；前端 `/jobs` 作业中心页。迁移 `b8c9d0e1f2a3`（合并双 head）。测试 `test_job_center_t77.py` + 既有 ansible/automation。**约束保持**：JSON-only、无 pickle、inventory 无凭据。**后续切片**：AdHoc/临时命令、Variable 参数化、周期调度、runas 执行身份 | 中 |
...
| M6：运维与平台治理 | 3-4 周 | #t77 + #t78 + #t79 | 🟡 #t77 首切片完成；#t78/#t79 未开始 | 收口阶段 |
...
> 因此后续排序建议为：**#t78 审计类型补全**（#t66/#t67/#t68、#t69/#t72、**#t73**、**#t74**、**#t76** 与 **#t77 作业中心首切片** 已落地；#t75 通知渠道若未合入可并行；#t70 建议独立立项；#t77 的 AdHoc/周期/runas 可后续切片），而非按里程碑编号顺序推进。
...
| 15 | 作业中心 | 🟡 JSON-only 队列 + worker + Ansible runner + **Playbook/Job/JobExecution 目录与 `/jobs` UI（#t77 首切片）** | 临时命令 AdHoc、Variable 参数化、周期任务、runas 执行身份策略 | 是 | P4 #t52 → **P6 #t77** |
