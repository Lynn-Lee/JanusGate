...
| 任务 ID | 任务 | Owner | 范围 | 优先级 |
|---------|------|-------|------|--------|
| **#t77** | 作业中心 | backend + frontend | **✅ 已完成**：`JobPlaybook` / `JobVariable` / `Job` / `JobExecution`；迁移 `g7h8i9j0k1l2` 合并 alembic heads `e5f6a7b8c9d0` + `f6b0d4c2e815`。`job.adhoc` 入 `ALLOWED_JOB_TYPES`；Playbook 作业复用 `ansible.playbook` + extra vars / runas / 目录 YAML。API `/api/v1/job-center/`（playbooks、variables、jobs、executions、`scheduler/tick`）；前端 `/jobs`。**约束已守住**：JSON-only，无 pickle（P0#10 不得回退）；敏感 extra vars 拒绝；Playbook 路径逃逸拒绝；adhoc 禁止 shell 模块与元字符；runas 只带 `account_id`。**作业中心首切片已 QA SHIP**；完整 cron DSL、更多 Ansible 模块可后续切片 | 中 |
...
> 因此后续排序建议为：**#t78 审计类型补全与会话高级能力**（#t66/#t67/#t68、#t69/#t72、**#t73 AccountTemplate / account.verify**、**#t74 asset_grant / command_review TicketFlow**、**#t76 OIDC 登录** 与 **#t77 作业中心** 已 QA SHIP；#t75 通知渠道扩展若尚未合入可并行；#t70 建议独立立项），而非按里程碑编号顺序推进。
