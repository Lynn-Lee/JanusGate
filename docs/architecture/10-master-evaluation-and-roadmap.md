...
| **#t59** | 文档站、安装手册、管理员手册、API 文档 | technical-writer | 文档站 foundation 已启动：`docs/site/index.md` 提供安装、管理员和 API 文档入口；`docs/site/install.md` 覆盖 Compose、Helm、多副本前置条件和回滚入口；`docs/site/admin.md` 覆盖管理面、License / Edition、安全边界、审计和运维门禁；`docs/site/api.md` 汇总稳定 API 分组并指向 `scripts/export-openapi-json.sh`；OpenAPI 自动生成 foundation 已接入 CI，通过 FastAPI `app.openapi()` 导出 `docs/site/openapi.json`，避免手写 schema 漂移；当前已补静态站点发布 smoke，`scripts/build-docs-site.sh` 会生成包含 Markdown、`openapi.json` 和 `manifest.json` 的 `dist/docs-site` 发布包并接入 CI；后续补截图化管理员手册和更完整的操作 runbook |
