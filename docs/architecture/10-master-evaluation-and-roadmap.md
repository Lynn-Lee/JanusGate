...
14. **License / Edition 边界**：Phase 5 #t58 已启动后端 feature flag 与 license 摘要 foundation：默认 community edition 启用基础能力；enterprise 配置必须携带 HMAC 校验通过且未过期的 license 才启用声明的 enterprise feature，缺失、非法、过期或签名不匹配时 fail-closed 回退 community；`GET /api/v1/admin/license-summary` 仅允许 admin 读取 configured/effective edition、license 状态和 feature 列表，不返回 license key、签名 secret 或原始 payload。完整 license lifecycle、管理 UI、离线公钥验签或外部商业授权服务仍待后续切片。
...
| **#t58** | 管理后台、License/Edition 边界 | architect + backend | Feature Flag 与 License 摘要 foundation 已启动：`JANUSGATE_EDITION=community|enterprise` 控制期望版本，默认 community 启用 `core_pam`、`workflow_jit`、`audit_reports`；enterprise 只有在 `JANUSGATE_LICENSE_KEY` 通过 `JANUSGATE_LICENSE_SIGNING_SECRET` HMAC-SHA256 校验且未过期时才生效，否则 fail-closed 回退 community；`GET /api/v1/admin/license-summary` 仅 admin 可读，不泄露 license key、签名 secret 或原始 payload；后续补完整 license lifecycle、管理后台 UI、离线公钥验签或外部商业授权服务 |
