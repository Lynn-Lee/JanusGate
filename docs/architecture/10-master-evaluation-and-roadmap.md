...
11. **性能压测与容量模型**：Phase 5 #t55 已启动容量模型 smoke foundation，`scripts/phase5-capacity-model-smoke.sh` 可用真实压测摘要或默认样例输入校验 core API p95、错误率和 `capacity_headroom_percent` SLO；`docs/performance/phase5-capacity-model.md` 记录当前 SLO、输入字段、容量余量公式与下一步真实环境压测证据要求。当前已补真实压测脚本 foundation：`scripts/phase5-core-api-load-test.sh` 可对目标环境执行认证 GET endpoint mix，并把 RPS、p95 和错误率聚合结果喂给容量模型 smoke；目标环境元数据、容量曲线和原始结果归档仍待后续切片。
