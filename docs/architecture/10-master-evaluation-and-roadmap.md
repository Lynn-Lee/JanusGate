...
| 19 | K8s 容器纳管 | 🟡 exec 通道（WebSocket v4.channel）+ namespace 作用域强制 + API Server TLS 强校验 + **生产 `AssetVaultSessionConnectionResolver` K8s 路径（QA SHIP）** | 云资产与 k8s 协议模型、短期 token 签发（TokenRequest API）、交互式 PTY exec | 是 | **P6 #t68、#t72** |
