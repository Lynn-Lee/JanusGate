...
| **#t71** | 数据库协议代理 | backend | **🟡 PG+MySQL 切片已完成**：PostgreSQL Simple Query（`postgres_proxy.py`）与 MySQL COM_QUERY + `mysql_native_password`（`mysql_proxy.py`）；`DatabaseVaultSessionConnectionResolver` 支持 `postgresql`/`mysql`/`mariadb`；SQL 审计复用 #t46 + #t65 执行前过滤与结果脱敏。进程内 wire server 测试，无外部依赖。**仍待**：prepared/extended query、交互式 REPL、per-account 库名、Oracle/SQL Server 等 | 中 |
...
| M3：真实连接通道 | 6-8 周 | #t69 + #t70 + #t71 + #t72 | 🟡 #t69/#t72/#t71(PG+MySQL) 完成；#t70 未开始 | 风险最高，#t70 图形通道为其中最重 |
...
> 因此后续排序建议为：**#t73 账号自动化** 或 **#t70 RDP/VNC 图形通道**（独立立项）（M3 #t71 PG+MySQL 代理已 SHIP），而非按里程碑编号顺序推进。
