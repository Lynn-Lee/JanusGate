...
| 任务 ID | 任务 | Owner | 范围 | 优先级 |
|---------|------|-------|------|--------|
| **#t63** | RBAC 角色与权限体系 | architect + backend | `Role` / `RoleBinding` 模型，system / org 双 scope；对象级 `Permission`；内置角色（系统管理员 / 组织管理员 / 审计员 / 普通用户）；菜单权限；与 §4 现有 `admin` / `workflow:admin` 字符串判断的迁移路径。**约束**：单一角色模型，禁止 edition 条件分支（对应关闭 P1#11）。**当前进度（部分完成）**：`Role`（自定义角色落库）/ `RoleBinding`（system / org scope）模型 + 迁移 `a7f4c9e21b83` 已落地；四个内置角色（系统管理员 / 组织管理员 / 审计员 / 普通用户）在代码中声明并与历史 `ADMIN_CONSOLE_PERMISSIONS` / `MVP_CONSOLE_PERMISSIONS` 权限集对齐；`RbacService.resolve_effective_permissions` 已接入登录 / 2FA / refresh 的 token 签发，规则为「`is_superuser` 回退基线 ∪ 显式角色绑定权限」，因此接入不削弱既有账号权限、且平滑迁移历史字符串权限；`GET/POST /api/v1/rbac/roles` 与 `GET/POST/DELETE /api/v1/rbac/role-bindings` 已提供租户隔离管理 API（读 `rbac:read`、写 `rbac:admin`，跨租户 404），所有查询走 `scoped_select`（提前落实 P2#6）。**仍待后续切片**：用户组（`user_group`）主体与成员管理、对象级/菜单权限精细化、组织切换 UI、以及把散落在各路由的字符串权限判断逐步替换为角色驱动校验 | 高 |
...
| M1：授权与访问控制内核 | 4-6 周 | #t63 + #t64 + #t65 | 🟡 #t64 已完成；#t65 命令过滤+脱敏+接线已 SHIP（登录 ACL 未做）；#t63 基础切片已 SHIP（Role/RoleBinding + 内置角色 + 权限解析接入 token + 管理 API；用户组/菜单权限未做） | 差距最大，优先级最高 |
...
> 因此后续排序建议为：**#t65 登录/资产登录/连接方式 ACL → #t63 用户组/菜单权限深化**（#t64 资产授权、#t65 命令过滤/脱敏接线、#t63 RBAC 基础切片已 SHIP），而非按里程碑编号顺序推进。
...
| 3 | RBAC 角色权限 | 🟡 `Role` / `RoleBinding` 模型 + 四个内置角色 + system/org scope + 有效权限解析接入 token 签发 + 租户隔离管理 API | 用户组主体与成员管理、对象级/菜单权限精细化、组织切换 UI、各路由字符串判断替换为角色驱动 | 是 | **P6 #t63** |
...
> v2.4 的 17 🟡 / 2 ⬜ 中，#7 ACL、#19 K8s 容器纳管和 #6 资产授权已纳入部分实现；随后 #t63 基础切片落地，#3 RBAC 角色权限由 ⬜ 转 🟡，当前为 18 🟡 / 1 ⬜，剩余唯一 ⬜ 为 #16 标签体系。
...
| | 安全重构完成度 | 功能替代完成度 |
|---|---|---|
| 当前（v2.4） | P0 17/20 ≈ 85% | 0/19 等价，18/19 部分 |
...
安全侧剩余 2 项 P0 全部收敛到 #t76 SSO；#t65 判定已接到 SSH/K8s/PTY 执行前。功能侧 #t64 资产树与 AssetPermission 已完成，#t63 RBAC 基础切片（角色/绑定/内置角色/权限解析/管理 API）已 SHIP；下一刀是 #t65 登录/资产登录/连接方式 ACL 与 #t63 的用户组/菜单权限深化（路由默认仍为 `NoopConnectorScheduler`，生产 resolver 属 #t69）。
