...
| **#t78** | 审计类型补全与会话高级能力 | backend + frontend | **🟡 文件传输 + 操作/改密 + 活动/在线会话已完成**：`FileTransferLog`（迁移 `f6a7b8c9d0e1`）、`OperateLog` / `PasswordChangeLog`（迁移 `a8b9c0d1e2f3`）、`ActivityLog` / `OnlineUserSession`（迁移 `b9c0d1e2f3a4`）；每条日志先写入 #t61 hash chain 再回指 `audit_event_id`；`HttpFileTransferEventSink` 对接 #t69 SFTP sink；资产/账号写操作、成功改密与交互式登录自动分类入库；管理员可结束在线会话；前端 `/audits` 只读列表。**约束**：全部审计类型并入 #t61 的 hash chain 与 WORM 归档。**仍待**：作业日志、会话共享与监控联机、端点路由、命令/录像多存储后端（含 ES） | 中 |
...
| M6：运维与平台治理 | 3-4 周 | #t77 + #t78 + #t79 | 🟡 #t78 FTPLog + 操作/改密 + 活动/在线会话 | 收口阶段；#t77/#t75 另有未合并 PR |
...
| 13 | 审计 | 🟡 统一审计事件 + **已持久化 append-only** + 库层强制有序 hash chain + SIEM + 合规报表 + WORM 归档 + **文件传输日志 + 操作/改密 + 活动/在线会话（#t78）** | 作业日志；会话共享与存储后端 | 是 | P1 已有 → **P6 #t61、#t78** |
