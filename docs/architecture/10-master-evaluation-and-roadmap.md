...
> **#t69 / #t72 生产 SessionConnectionResolver 已 QA SHIP**（`AssetVaultSessionConnectionResolver` 为默认 HTTP 装配；K8s 走 API URL + 预置 CA + namespace + Vault token）。本次修订按实际代码状态回写
...
> - 两条路线曾在 **#t69 真实连接通道**上强耦合；该单点关键路径已于 v2.4 解除，#t69 / #t72 生产 resolver 亦已 QA SHIP（§14.4）
...
6. **通知/WebHook 集成**：Phase 4 #t47 已启动租户隔离的 `WebhookEndpoint` 持久化模型与 `GET/POST /api/v1/webhook-endpoints/` 管理 API；响应不返回 signing secret 明文或摘要。当前已补 `NotificationRule` 持久化模型与 `GET/POST /api/v1/notification-rules/` 管理 API，规则必须引用当前租户 active WebHook endpoint；并补 `NotificationDelivery` 队列记录与租户隔离的入队/列表 API。`NotificationDeliveryWorker` 已提供到期投递、失败重试与最大次数后 dead-letter 状态机，sender 只接收已脱敏 payload。真实外部 HTTP sender 已由 #t47 落地，IM / 站内信渠道与系统消息订阅已由 #t75 QA SHIP，多级审批已由 #t74 完成。
...
| **#t75** | 通知渠道扩展 | backend + frontend | **✅ 已完成**：在 #t47 WebHook / 通知规则 / 投递队列上扩展 `channel_type`（钉钉 / 飞书 / Lark / 企微 / Slack / SMS / 邮件 / 站内信）；IM 只允许官方 host，机器人 token 从 URL 剥离后 AES-256-GCM 落库；邮件/短信走 HTTPS 网关 Bearer，不直连 SMTP；`SystemMessageSubscription` + `POST /api/v1/notification-events/` 扇出；站内信必须指定接收人，规则不能绑 inbox。**通知渠道扩展已 QA SHIP**（沿用 #t47 脱敏 payload 与 dead-letter 契约）。设置页「通知渠道」「系统消息订阅」「站内信」。测试 `test_notification_channels_t75.py` | 中 |
...
| 任务 ID | 任务 | Owner | 范围 | 优先级 |
|---------|------|-------|------|--------|
| **#t77** | 作业中心 | backend + frontend | **✅ 已完成**：`JobPlaybook` / `JobVariable` / `Job` / `JobExecution`；迁移 `f7a8b9c0d1e2`（接 #t75 `f6a7b8c9d0e1`）。`job.adhoc` 入 `ALLOWED_JOB_TYPES`；Playbook 作业复用 `ansible.playbook` + extra vars / runas / 目录 YAML。API `/api/v1/job-center/`（playbooks、variables、jobs、executions、`scheduler/tick`）；前端 `/jobs`。**约束已守住**：JSON-only，无 pickle（P0#10 不得回退）；敏感 extra vars 拒绝；Playbook 路径逃逸拒绝；adhoc 禁止 shell 模块与元字符；runas 只带 `account_id`。**作业中心已 QA SHIP**；完整 cron DSL、更多 Ansible 模块可后续切片 | 中 |
...
> 因此后续排序建议为：**#t78 审计类型补全与会话高级能力**（#t66/#t67/#t68、#t69/#t72、**#t73 AccountTemplate / account.verify**、**#t74 asset_grant / command_review TicketFlow**、**#t75 通知渠道扩展**、**#t76 OIDC 登录** 与 **#t77 作业中心** 已 QA SHIP；#t70 建议独立立项），而非按里程碑编号顺序推进。
...
| 14 | 通知 | 🟡 WebHook endpoint + 通知规则 + 投递队列 + 重试/死信 + HTTPS sender + **IM/网关/站内信渠道与系统消息订阅（#t75 QA SHIP）** | 真实运营商 SMS / SMTP 直连、更广订阅过滤器 | 是 | P4 #t47 → **P6 #t75** |
| 15 | 作业中心 | 🟡 Playbook/变量/作业/临时命令/周期 tick + JSON-only 队列 + Ansible runner + JobExecution（**#t77 QA SHIP**） | 完整 cron DSL、更多 Ansible 模块、作业日志并入 #t78 | 是 | P4 #t52 → **P6 #t77** |
...
- **#t65 命令过滤/脱敏已接到 SSH exec、SSH PTY 与 K8s exec 执行前**；**登录 ACL / 资产登录 ACL / 连接方式 ACL overlay 已 SHIP**（交互式登录走 `evaluate_login`，connect 叠在 AssetPermission 之后；时段按租户时区求值，默认 Asia/Singapore）。**#t69 / #t72 生产 SessionConnectionResolver 已 QA SHIP**：默认 HTTP 装配为 `AssetVaultSessionConnectionResolver`（资产注册表 + Vault），测试仍注入 Noop/Fake；SSH 走已批准主机密钥（scan-then-compare、fail-closed、无 TOFU）；K8s 走 API URL + 预置 CA + 单一 namespace + Vault token（仅内存）；审批在现有 WorkflowPage（未知「确认这台主机」/变更「这台主机的密钥变了」）；未批准/拒绝/不匹配「无法连接」；缺 HTTPS/CA 为「无法连接（需要 HTTPS 和 CA）」；连接列表展示 k8s（#t72 建连弹层选 Pod）。
...
安全侧 P0#12 / P0#14 / P1#7 已由 **#t76 OIDC 登录 QA SHIP** 关闭（强制 PKCE + state、HTTPS/`verify=True`、`safe_next_url`）；#t65 判定已接到 SSH/K8s/PTY 执行前与交互式登录/资产连接 overlay。功能侧 #t63 RBAC、#t64 资产树与 AssetPermission、#t65 overlay ACL、#t66 资产类型与协议、**#t67 网域与网关 ProxyJump 已 QA SHIP**、**#t68 TokenRequest 已 QA SHIP**、**#t69 / #t72 生产 SessionConnectionResolver 已 QA SHIP**、**#t73 AccountTemplate / account.verify 已 QA SHIP**、**#t74 asset_grant / command_review TicketFlow 已 QA SHIP**、**#t75 通知渠道扩展已 QA SHIP**、**#t76 OIDC 登录已 QA SHIP**、**#t77 作业中心已 QA SHIP**；下一刀是 **#t78 审计类型补全与会话高级能力**。
