...
7. **审计投递可靠性与报表中心**：SIEM 投递失败不阻断；Phase 4 #t49 已启动当前租户审计报表汇总 API，可靠队列/告警/合规报表 UI 仍待后续切片
...
| **#t49** | SIEM/告警/报表中心 | backend + frontend | 当前租户审计报表汇总 API `GET /api/v1/audits/reports/summary` 已启动，返回 total、severity、category、SIEM delivery 状态和高危计数且不泄露 metadata；后续补可靠队列、告警、合规报表和 Dashboard | 中 |
