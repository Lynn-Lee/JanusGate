...
**Phase 4 增强**：
- SSH CA / 临时证书服务已启动后端模型与签发/撤销服务契约；真实 OpenSSH signer、连接器信任分发和 REST API 待后续 #t44 切片
- mTLS 证书绑定
- key rotation
...
| **#t44** | SSH CA / 临时证书 | backend + security | `SshCertificateAuthority` / `SshCertificate` 模型 + `Asset.trusted_ssh_ca_id` 信任配置字段 + 后端签发/撤销服务契约已启动；后续补真实 OpenSSH signer、REST API、连接器信任分发与前端入口 | 高 |
