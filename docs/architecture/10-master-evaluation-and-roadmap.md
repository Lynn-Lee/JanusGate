...
| **#t54** | 审计合规报表 | backend + frontend | 合规报表导出基础已启动：`GET /api/v1/audits/reports/compliance` 按当前租户和模板返回事件 ID 列表、hash chain 起止、报告期间、HMAC-SHA256 报表签名、正式 JSON 导出格式元数据和安全下载文件名，不返回 metadata、message、resource_id、session_id 或任何凭据相关明细字段；当前导出会写入 append-only WORM 归档元数据 `worm_record_id`、`worm_sequence_number` 与基于无敏感摘要的 `worm_content_hash`；前端 `/audits` 已提供 SOC2 合规报表 JSON 下载入口，使用后端返回的 vendor JSON media type 与安全文件名，并展示事件数与报表签名摘要，不展示原始审计 metadata；外部 WORM 存储和外部签章仍待后续切片 |
