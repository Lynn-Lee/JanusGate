...
| **#t75** | 通知渠道扩展 | backend | **✅ 已完成**：在 #t47 WebHook / 通知规则 / 投递队列基础上扩展 `channel_type`（webhook / dingtalk / feishu / lark / wecom / slack / sms / email / inbox）；IM 官方 host 白名单、URL 禁止 query 凭据、渠道凭据 AES-256-GCM；`SystemMessageSubscription` + `POST /api/v1/notification-events/` 扇出；站内信 `InAppMessage`；邮件 SMTPS/STARTTLS；SMS HTTPS 网关；设置页「通知渠道 / 系统消息订阅 / 站内信」。**通知渠道扩展已 QA SHIP**（沿用脱敏 payload 与 dead-letter 契约）。真实运营商 SMS 专有协议与 SMTP 账号池可后续切片 | 中 |
...
| 14 | 通知 | 🟡 WebHook endpoint + 通知规则 + 投递队列 + 重试/死信 + HTTPS sender + **IM/SMS/邮件/站内信 sender + 系统消息订阅（#t75 QA SHIP）** | 真实运营商 SMS 专有协议、SMTP 账号池 | 是 | P4 #t47 → **P6 #t75** |
