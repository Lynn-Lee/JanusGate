...
| 任务 ID | 任务 | Owner | 范围 | 优先级 |
|---------|------|-------|------|--------|
| **#t77** | 作业中心 | backend + frontend | **✅ 已完成**：`OpsPlaybook` / `OpsJob` / `OpsJobVariable` / `OpsJobExecution`（表 `ops_*`）；迁移 `a8b9c0d1e2f3` 合并双 head。API 前缀 `/api/v1/ops/`；队列 `ops.job`，payload 仅 `execution_id` + `check_mode`。临时命令走内置 `janusgate-adhoc.yml`；extra vars 仅标量并拒绝敏感键 / Jinja；runas 为租户内 active SSH 账号，inventory 最多带 `ansible_user`。Adhoc 入队前 `evaluate_command`，DENY/REVIEW 均拒绝。Cron 5 段 UTC + `POST /ops/scheduler/tick`。前端 `/jobs`「作业中心」。测试 `test_job_center_t77.py` + `mvp-pages.test.tsx`。**作业中心已 QA SHIP**（JSON-only，无 pickle） | 中 |
...
> 因此后续排序建议为：**#t78 审计类型补全与会话高级能力**（#t66/#t67/#t68、#t69/#t72、**#t73 AccountTemplate / account.verify**、**#t74 asset_grant / command_review TicketFlow**、**#t76 OIDC 登录** 与 **#t77 作业中心** 已 QA SHIP；#t75 通知渠道可并行合入，#t70 建议独立立项），而非按里程碑编号顺序推进。
...
| 15 | 作业中心 | 🟡 JSON-only 队列 + worker + Ansible runner + **作业/Playbook/临时命令/周期任务/runas（#t77 QA SHIP）** | 图形作业编排、命令复核工单联动、更丰富的 runner 输出 | 是 | P4 #t52 → **P6 #t77** |
