...
- 作业中心：`/api/v1/job-center/playbooks/`、variables、jobs、executions 与 `scheduler/tick`（JSON-only，无 pickle）；详见[作业中心](job-center.md)。
...
- 通知渠道：`/api/v1/webhook-endpoints/`、`/api/v1/system-message-subscriptions/`、`/api/v1/notification-events/` 与 `/api/v1/in-app-messages/`；详见[通知渠道扩展](notification-channels.md)。
