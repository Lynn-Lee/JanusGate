...
- Audit：`/api/v1/audits/events`、分类日志（operate / activity / file-transfers / password-changes / job-logs）、在线会话、`/api/v1/audits/reports/summary`、`/api/v1/audits/reports/compliance`。文件传输入库见 `POST /api/v1/connectors/{connector_id}/file-transfers`，详见[审计类型](audit-types.md)。
