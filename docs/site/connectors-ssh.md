...
- **生产装配已 SHIP**：默认 HTTP 装配使用 `AssetVaultSessionConnectionResolver`（资产注册表 + Vault + 已批准主机密钥）替换 `NoopConnectorScheduler`；测试仍注入 `InMemorySessionConnectionResolver` 或 Noop。仅 SSH（exec / interactive / sftp）。主机密钥 scan-then-compare、fail-closed、无 TOFU。审批留在现有 WorkflowPage：未知「确认这台主机」、变更「这台主机的密钥变了」，无 type-to-confirm。未批准/拒绝/不匹配一律「无法连接」。连接列表隐藏 k8s。机制与端到端测试见 `tests/connectors/test_session_runtime.py` 与 `tests/connectors/test_asset_vault_resolver.py`。
