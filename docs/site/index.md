...
- [操作 Runbook](runbooks.md)：发布、多副本 smoke、回滚和密钥处理清单；配套 `fixtures/operation-runbook-evidence.json` 固定操作证据归档字段。
...
- 静态站点发布包由 `scripts/build-docs-site.sh dist/docs-site` 生成，包含本目录 Markdown、管理员截图资产、截图 fixture、真实截图归档合约、操作 runbook evidence manifest、`openapi.json` 和 `manifest.json`。
