...
Phase 6 已交付的专项文档同样纳入静态发布包：[`site/connectors-ssh.md`](site/connectors-ssh.md)（#t69 SSH / SFTP / PTY 通道的安全约束与命令事件契约）、[`site/connectors-k8s.md`](site/connectors-k8s.md)（#t72 K8s exec 通道的 namespace 作用域、TLS 强校验与凭据边界）、[`site/connectors-db.md`](site/connectors-db.md)（#t71 PostgreSQL / MySQL 协议代理、SQL 命令事件与脱敏）、[`site/acl-command-filter.md`](site/acl-command-filter.md)（#t65 命令过滤 ACL 与命令组的判定语义和租户 scope 加载）、[`site/acl-data-masking.md`](site/acl-data-masking.md)（#t65 数据脱敏规则的累计应用语义）、[`site/platform-governance.md`](site/platform-governance.md)（#t79 资源标签、动态配置、用户偏好、泄露密码库与报表目录）。这六篇随 `scripts/build-docs-site.sh` 一并拷贝并登记进 `manifest.json` 的 pages 列表。
