...
## 任务收口文档清单（每个功能切片完成后必须同步）

1. **研发总计划 / roadmap**：`architecture/10-master-evaluation-and-roadmap.md`（任务状态、里程碑、§14 完成度矩阵、版本历史）
2. **API 契约**：`api-contract.md`（新增/变更的稳定 API 与错误语义）
3. **专项设计 / 产品说明**：`site/<feature>.md`（管理员/集成方可读的行为与边界）
4. **系统架构参考**：`architecture/02-*.md` 等（与统一决策/连接器/Vault 等子系统的边界变更时）
5. **项目索引**：根目录 `README.md`、`site/index.md`、`docs/README.md`（入口与交叉引用）
6. **静态发布包**：`scripts/build-docs-site.sh` 的 `pages` / `manifest.json`（纳入文档站 smoke）

代码合并前须将上述文档与实现一并提交 Git，保持本地与远端一致。
