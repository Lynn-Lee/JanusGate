...
Phase 6 已交付的专项文档同样纳入静态发布包：[`site/connectors-ssh.md`](site/connectors-ssh.md)（#t69 SSH / SFTP / PTY 通道的安全约束与命令事件契约）、[`site/connectors-k8s.md`](site/connectors-k8s.md)（#t72 K8s exec 通道的 namespace 作用域、TLS 强校验与凭据边界）、[`site/k8s-management.md`](site/k8s-management.md)（#t68 K8s 集群纳管、TokenRequest 与生产 resolver 装配）、[`site/connectors-database.md`](site/connectors-database.md)（#t71 PostgreSQL Simple Query 代理与 SQL 审计）、[`site/acl-command-filter.md`](site/acl-command-filter.md)（#t65 命令过滤 ACL 与命令组的判定语义和租户 scope 加载）、[`site/acl-data-masking.md`](site/acl-data-masking.md)（#t65 数据脱敏规则的累计应用语义）。这六篇随 `scripts/build-docs-site.sh` 一并拷贝并登记进 `manifest.json` 的 pages 列表。
