...
Phase 5 #t59 文档站 foundation 从 [`docs/site/index.md`](site/index.md) 开始，覆盖安装手册、管理员手册和 API 文档入口；OpenAPI JSON 通过 `scripts/export-openapi-json.sh` 自动生成。静态发布包可通过 `scripts/build-docs-site.sh dist/docs-site` 生成，CI 会执行该 smoke，确保文档入口和 OpenAPI schema 可一起交付。
