"""连接器侧 MySQL COM_QUERY 代理通道（#t71 M3：数据库协议代理）。
...
class MysqlTarget:
    ...
    :param database: 默认库名。
    :param username: 登录用户。
    :param require_tls: 为 ``True`` 时在握手后发 SSLRequest 并强制校验证书。
...
def _mysql_native_password_scramble(password: str, salt: bytes) -> bytes:
    """``mysql_native_password`` 线协议加盐；SHA-1 为协议强制，不是本地 KDF。"""
...
class MysqlQueryChannel:
    ...
    async def run_query(
        ...
        *,
        ...
        """执行一条 COM_QUERY：先策略判定，再连远端，结果脱敏后投递命令事件。"""
    ...
    async def _complete_handshake(
        ...
        if self._ssl is not None:
            ssl_caps = capabilities | _CLIENT_SSL
            ssl_request = bytearray()
            ssl_request += struct.pack("<I", ssl_caps)
            ssl_request += struct.pack("<I", 16777216)
        ...
        await _write_packet(writer, seq, bytes(response))
