        ...
        if msg_type == b"Q":
            ...
            if "fail" in sql.lower():
                writer.write(
                    _pack_message(
                        b"E",
                        b"SERROR\x00C42501\x00Mpermission denied\x00\x00",
                    )
                )
                await writer.drain()
...
async def _pg_handler(
    ...
    *,
    ...
    await _pg_session(reader, writer, captured=captured)
...
@pytest.mark.asyncio
async def test_postgres_query_result_is_masked(pg_server) -> None:
    host, port, _ = pg_server
    sink = _RecordingSink()
    channel = await PostgresQueryChannel.open(
        PostgresTarget(host=host, port=port, database="appdb", username="alice"),
...
@pytest.mark.asyncio
async def test_postgres_tls_query_with_pinned_ca() -> None:
    ...
    async def handler(reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        _length, code = struct.unpack("!II", header)
...
@pytest.mark.asyncio
async def test_session_runtime_opens_postgres_channel(pg_server) -> None:
    host, port, _ = pg_server
    ...
    resolver.register(
        ...
        spec=SessionConnectionSpec(
            ...
            postgres_target=PostgresTarget(
                host=host, port=port, database="appdb", username="alice"
            ),
            postgres_credential=PostgresCredential(password="s3cret"),
        ),
    )
    runtime = ConnectorSessionRuntime(resolver, command_policy=_guard(_FakePolicy()))
    ...
    assert record.mode is ConnectorSessionMode.DB_POSTGRESQL
    event = await record.channel.run_query("SELECT 1", _RecordingSink(), sequence=0)
    assert event.exit_code == 0
    await runtime.close(record.connector_session_id)
