...
def upgrade() -> None:
    ...
    with op.batch_alter_table("accounts") as batch_op:
        ...
        batch_op.add_column(sa.Column("last_verify_message_id", sa.String(length=120), nullable=True))
        batch_op.add_column(sa.Column("last_verify_at", sa.DateTime(timezone=True), nullable=True))
        batch_op.create_index(op.f("ix_accounts_template_id"), ["template_id"], unique=False)
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("accounts") as batch_op:
        ...
        batch_op.drop_index(op.f("ix_accounts_template_id"))
        batch_op.drop_column("last_verify_at")
        batch_op.drop_column("last_verify_message_id")
