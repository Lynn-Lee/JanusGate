...
down_revision: tuple[str, str] | str | None = ("e5f6a7b8c9d0", "f6b0d4c2e815")
...
def upgrade() -> None:
    ...
    op.add_column("webhook_endpoints", sa.Column("credential_encrypted", sa.Text(), nullable=True))
    ...
    with op.batch_alter_table("notification_deliveries") as batch_op:
        batch_op.alter_column("notification_rule_id", existing_type=sa.Integer(), nullable=True)
    ...
    op.create_index(
        op.f("ix_in_app_messages_delivery_id"),
        "in_app_messages",
        ["delivery_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("notification_deliveries") as batch_op:
        batch_op.alter_column("notification_rule_id", existing_type=sa.Integer(), nullable=False)
    op.drop_index(
        op.f("ix_notification_deliveries_recipient_user_id"),
